...
import { createInvoiceStatus, deleteInvoiceStatus, fetchInvoiceStatus, updateInvoiceStatus, updateInvoiceType } from "../../api/Invoice";
    ...
    return (
        <div style={{ height: 750, width: "100%" }}>
            <ActionBar
                ...
                title={"ایجاد وضعیت  صورتحساب"}
                apiFunction={createInvoiceStatus}
            />
            {open && <EditComponent
                formConfig={formData}
                title={"ویرایش"}
                handleClose={() => setOpen(!open)}
                apiFunction={updateInvoiceStatus}
                open={open}
            />}
            {deleteDialog && selectId && (
                <DeleteConfirmation show={deleteDialog} handleClose={() => setDeleteDialog(false)} apiFunction={deleteInvoiceStatus} id={selectId} />
            )}
            <DataGridComponent
                key={pageSize}
                columns={[
