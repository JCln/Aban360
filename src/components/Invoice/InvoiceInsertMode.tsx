...
import { createInvoiceInsertMode, deleteInvoiceInsertMode, fetchInvoiceInsertMode, updateInvoiceInsertMode } from '../../api/Invoice/index';

const InvoiceInsertMode = () => {
    ...
    useEffect(() => {
        setLoading(true)
        fetchInvoiceInsertMode().then(res => {
            if (res?.data) {
                setRows(res.data)
                setLoading(false)
            }
    ...
    return (
        <div style={{ height: 750, width: "100%" }}>
            <ActionBar
                ...
                apiFunction={createInvoiceInsertMode}
            />
            {open && <EditComponent
                formConfig={formData}
                title={"ویرایش"}
                handleClose={() => setOpen(!open)}
                apiFunction={updateInvoiceInsertMode}
                open={open}
            />}
            {deleteDialog && selectId && (
                <DeleteConfirmation show={deleteDialog} handleClose={() => setDeleteDialog(false)} apiFunction={deleteInvoiceInsertMode} id={selectId} />
            )}
            <DataGridComponent
                key={pageSize}
                columns={[
...
export default InvoiceInsertMode