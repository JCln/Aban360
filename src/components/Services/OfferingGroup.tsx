...
import { createOfferingGroup, deleteOfferingGroup, fetchOfferingGroup, updateOfferingGroup } from "../../api/Services";
export const useOfferingGroup = () => useQuery("OfferingGroup", fetchOfferingGroup);
    ...
    return (
        <div style={{ height: 750, width: "100%" }}>
            <ActionBar
                ...
                apiFunction={createOfferingGroup}
            />
            {open && <EditComponent
                formConfig={formData}
                title={"ویرایش"}
                handleClose={() => setOpen(!open)}
                apiFunction={updateOfferingGroup}
                open={open}
            />}
            {deleteDialog && selectId && (
                <DeleteConfirmation show={deleteDialog} handleClose={() => setDeleteDialog(false)} apiFunction={deleteOfferingGroup} id={selectId} />
            )}
            <DataGridComponent
                key={pageSize}
                columns={[
