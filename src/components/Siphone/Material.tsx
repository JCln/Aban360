...
import { createSiphonMaterial, deleteSiphonMaterial, fetchSiphonMaterial, updateSiphonDiameter, updateSiphonMaterial } from '../../api/Siphon/index';

const SiphoneMaterial = () => {
    ...
    return (
        <div style={{ height: 750, width: "100%" }}>
            <ActionBar
                ...
                title={"ایجاد جنس سیفون"}
                apiFunction={createSiphonMaterial}
            />
            {open && <EditComponent
                formConfig={formData}
                title={"ویرایش"}
                handleClose={() => setOpen(!open)}
                apiFunction={updateSiphonMaterial}
                open={open}
            />}
            {deleteDialog && selectId && (
                <DeleteConfirmation show={deleteDialog} handleClose={() => setDeleteDialog(false)} apiFunction={deleteSiphonMaterial} id={selectId} />
            )}
            <DatailGridComponent
                key={pageSize}
                columns={[
...
export default SiphoneMaterial