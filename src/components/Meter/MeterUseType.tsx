...
import { createMeterUseType, deleteMeterUseType, fetchMeterUseType, updateMeterUseType } from "../../api/Meter";
    ...
    return (
        <div style={{ height: 750, width: "100%" }}>
            <ActionBar
                ...
                title={"ایجاد نوع مصرف کنتور "}
                apiFunction={createMeterUseType}
            />
            {open && <EditComponent
                formConfig={formData}
                title={"ویرایش"}
                handleClose={() => setOpen(!open)}
                apiFunction={updateMeterUseType}
                open={open}
            />}
            {deleteDialog && selectId && (
                <DeleteConfirmation show={deleteDialog} handleClose={() => setDeleteDialog(false)} apiFunction={deleteMeterUseType} id={selectId} />
            )}
            <DatailGridComponent
                key={pageSize}
                columns={[
