...
import { createMeterMaterial, deleteMeterMaterial, fetchMeterMaterial, updateMeterMaterial } from "../../api/Meter";
    ...
    return (
        <div style={{ height: 750, width: "100%" }}>
            <ActionBar
                ...
                title={"ایجاد جنس کنتور"}
                apiFunction={createMeterMaterial}
            />
            {open && <EditComponent
                formConfig={formData}
                title={"ویرایش"}
                handleClose={() => setOpen(!open)}
                apiFunction={updateMeterMaterial}
                open={open}
            />}
            {deleteDialog && selectId && (
                <DeleteConfirmation show={deleteDialog} handleClose={() => setDeleteDialog(false)} apiFunction={deleteMeterMaterial} id={selectId} />
            )}
            <DatailGridComponent
                key={pageSize}
                columns={[
