...
import { updateWaterMeterTagDefinition, fetchWaterMeterTagDefinition, createWaterMeterTagDefinition, deleteWaterMeterTagDefinition } from '../../api/Meter/index';

const WaterMeterTagDefinition = () => {
    ...
    useEffect(() => {
        setLoading(true)
        fetchWaterMeterTagDefinition().then(res => {
            if (res?.data) {
                setRows(res.data)
                setLoading(false)
            }
    ...
    return (
        <div style={{ height: 750, width: "100%" }}>
            <ActionBar
                ...
                title={"ایجاد برچسب ها کنتور "}
                apiFunction={createWaterMeterTagDefinition}
            />
            {open && <EditComponent
                formConfig={formData}
                title={"ویرایش"}
                handleClose={() => setOpen(!open)}
                apiFunction={updateWaterMeterTagDefinition}
                open={open}
            />}
...
export default WaterMeterTagDefinition