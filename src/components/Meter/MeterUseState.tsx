...
import { createUseState, deleteUseState, fetchUseState, updateUseState } from "../../api/Meter";
    ...
    return (
        <div style={{ height: 750, width: "100%" }}>
            <ActionBar
                ...
                apiFunction={createUseState}
            />
            {open && <EditComponent
                formConfig={formData}
                title={"ویرایش"}
                handleClose={() => setOpen(!open)}
                apiFunction={updateUseState}
                open={open}
            />}
            {deleteDialog && selectId && (
                <DeleteConfirmation show={deleteDialog} handleClose={() => setDeleteDialog(false)} apiFunction={deleteUseState} id={selectId} />
            )}
            <DatailGridComponent
                key={pageSize}
                columns={[
