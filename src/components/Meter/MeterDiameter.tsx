...
import { createMeterDiameter, deleteMeterDiameter, fetchMeterDiameter, updateMeterDiameter } from "../../api/Meter";
    ...
    return (
        <div style={{ height: 750, width: "100%" }}>
            <ActionBar
                ...
                title={"ایجاد قطر کنتور "}
                apiFunction={createMeterDiameter}
            />
            {open && <EditComponent
                formConfig={formData}
                title={"ویرایش"}
                handleClose={() => setOpen(!open)}
                apiFunction={updateMeterDiameter}
                open={open}
            />}
            {deleteDialog && selectId && (
                <DeleteConfirmation show={deleteDialog} handleClose={() => setDeleteDialog(false)} apiFunction={deleteMeterDiameter} id={selectId} />
            )}
            <DatailGridComponent
                key={pageSize}
                columns={[
                    ...
                    ],
                    {
                        ...
                        renderCell: (cellValues: any) => {
                            const onClick = (e: any) => {
                                setOpen(true)
                                setFormData([{ name: 'id', type: 'input', class: "text", inputType: "text", value: `${cellValues?.id}` },
