...
import { createMeterType, deleteMeterType, fetchMeterType, updateMeterType } from "../../api/Meter";
    ...
    return (
        <div style={{ height: 750, width: "100%" }}>
            <ActionBar
                ...
                title={"ایجاد نوع کنتور "}
                apiFunction={createMeterType}
            />
            {open && <EditComponent
                formConfig={formData}
                title={"ویرایش"}
                handleClose={() => setOpen(!open)}
                apiFunction={updateMeterType}
                open={open}
            />}
            {deleteDialog && selectId && (
                <DeleteConfirmation show={deleteDialog} handleClose={() => setDeleteDialog(false)} apiFunction={deleteMeterType} id={selectId} />
            )}
            <DatailGridComponent
                key={pageSize}
                columns={[
