...
import { createMeterProducer, deleteMeterProducer, fetchMeterProducer, updateMeterProducer } from "../../api/Meter";
    ...
    return (
        <div style={{ height: 750, width: "100%" }}>
            <ActionBar
                ...
                title={"ایجاد سازنده کنتور "}
                apiFunction={createMeterProducer}
            />
            {open && <EditComponent
                formConfig={formData}
                title={"ویرایش"}
                handleClose={() => setOpen(!open)}
                apiFunction={updateMeterProducer}
                open={open}
            />}
            {deleteDialog && selectId && (
                <DeleteConfirmation show={deleteDialog} handleClose={() => setDeleteDialog(false)} apiFunction={deleteMeterProducer} id={selectId} />
            )}
            <DatailGridComponent
                key={pageSize}
                columns={[
