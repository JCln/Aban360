...
import { createEstateBoundType, deleteEstateBoundType, fetchEstateBoundType, updateEstateBoundType } from '../../api/Property/index';
    ...
    return (
        <div style={{ height: 750, width: "100%" }}>
            <ActionBar
                ...
                title={"ایجاد محدوده"}
                apiFunction={createEstateBoundType}
            />
            {open && <EditComponent
                formConfig={formData}
                title={"ویرایش"}
                handleClose={() => setOpen(!open)}
                apiFunction={updateEstateBoundType}
                open={open}
            />}
            {deleteDialog && selectId && (
                <DeleteConfirmation show={deleteDialog} handleClose={() => setDeleteDialog(false)} apiFunction={deleteEstateBoundType} id={selectId} />
            )}
            <DataGridComponent
                key={pageSize}
                columns={[
