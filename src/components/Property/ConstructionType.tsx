...
import { createConstructionType, deleteConstructionType, fetchConstructionType, updateConstructionType } from '../../api/Property/index';
    ...
    return (
        <div style={{ height: 750, width: "100%" }}>
            <ActionBar
                ...
                title={"ایجادنوع سازه"}
                apiFunction={createConstructionType}
            />
            {open && <EditComponent
                formConfig={formData}
                title={"ویرایش"}
                handleClose={() => setOpen(!open)}
                apiFunction={updateConstructionType}
                open={open}
            />}
            {deleteDialog && selectId && (
                <DeleteConfirmation show={deleteDialog} handleClose={() => setDeleteDialog(false)} apiFunction={deleteConstructionType} id={selectId} />
            )}
            <DatailGridComponent
                key={pageSize}
                columns={[
                    ...[ID(10), TITLE(400),
                    { field: "estates", headerName: "", width: 150 },
                    ],
                    {
                        field: "edit",
                        headerName: "",
