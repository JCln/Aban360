...
import { createIndividualType, deleteIndividualType, fetchIndividualType, updateIndividualType } from "../../api/People";
    ...
    return (
        <div style={{ height: 750, width: "100%" }}>
            <ActionBar
                ...
                title={"ایجاد نوع شخصیت"}
                apiFunction={createIndividualType}
            />
            {open && <EditComponent
                formConfig={formData}
                title={"ویرایش"}
                handleClose={() => setOpen(!open)}
                apiFunction={updateIndividualType}
                open={open}
            />}
            {deleteDialog && selectId && (
                <DeleteConfirmation show={deleteDialog} handleClose={() => setDeleteDialog(false)} apiFunction={deleteIndividualType} id={selectId} />
            )}
            <DatailGridComponent
                key={pageSize}
                columns={[
