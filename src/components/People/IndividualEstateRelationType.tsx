...
import { createIndividualEstateRelationType, deleteIndividualEstateRelationType, fetchIndividualEstateRelationType, updateIndividualEstateRelationType } from "../../api/People";

const IndividualEstateRelationType = () => {
    ...
    useEffect(() => {
        setLoading(true)
        fetchIndividualEstateRelationType().then(res => {
            if (res?.data) {
                setRows(res.data)
                setLoading(false)
            }
    ...
    return (
        <div style={{ height: 750, width: "100%" }}>
            <ActionBar
                ...
                title={"ایجاد وضعیت اشخاص"}
                apiFunction={createIndividualEstateRelationType}
            />
            {open && <EditComponent
                formConfig={formData}
                title={"ویرایش"}
                handleClose={() => setOpen(!open)}
                apiFunction={updateIndividualEstateRelationType}
                open={open}
            />}
            {deleteDialog && selectId && (
                <DeleteConfirmation show={deleteDialog} handleClose={() => setDeleteDialog(false)} apiFunction={deleteIndividualEstateRelationType} id={selectId} />
            )}
            <DatailGridComponent
                key={pageSize}
                columns={[
...
export default IndividualEstateRelationType